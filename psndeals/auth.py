...
#https://auth.api.sonyentertainmentnetwork.com/login.do
...
def login(email, password):
    ...
    if result:
        ...
        click.secho('Problem with sign-in page.', err=True, fg='red')
        return False
...
def send_oauth(payload):
    click.secho('Getting token: {}'.format(payload))
    try:
        r = requests.post(OAUTH_URL, data=payload)
    except:
        click.secho('Could not connect to oauth server.', fg='red', err=True)
        return False
